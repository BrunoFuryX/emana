...
window.addEventListener(
    ...
    function (e) {
        if (config.minicart.actived) {
            store.dispatch(
                minicartFetch(
                    document.querySelector("html").getAttribute("data-store")
                )
            )
        } else {
            axios
                ...
                .then(data => {
                    let amount = data
                        .map(cart => parseInt(cart.Cart.quantity))
                        .reduce(
                    ...
                    let price = data
                        .map(
                            cart =>
                                parseFloat(cart.Cart.price) *
                                parseInt(cart.Cart.quantity)
                        )
                        .reduce(
                            (valorAnterior, valorAtual) =>
                                valorAnterior + valorAtual
                    ...
                    // Update Amount
                    Array.from(
                        document.querySelectorAll("[data-cart=amount]")
                    ).forEach(item => (item.innerHTML = amount))
                    Array.from(
                        document.querySelectorAll("[data-cart=price]")
                    ).forEach(item => (item.innerHTML = price))
